...
Every guide must contain a first level heading (e.g., `# Heading`), which will be the guide title. Section headings are second level headings (e.g., `## Section`).

The guide itself needs to be [written in Markdown](#writing-text). For each topic you want to include, you can either mention so on a line surrounded by whitespaces:
