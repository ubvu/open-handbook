...
#### Changing text in multiple locations

It happens from time to time that text that occurs in multiple locations needs to be changed. This can be the case when a URL to an external website changes; when this URL occurs in multiple places in the handbook, it can be easier to change it in one go, rather than several times, separately.
