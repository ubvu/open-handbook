---
title: "Publishing FAIR software"
...
This document provides some pointers that can help to set you up with a coding repository that follows the FAIR guiding principles, meaning Findability, Accessibility, Interoperability, and Reuse of digital assets. It is not meant to be an exhaustive guide for every step, but rather acts as a starting point and link to more exhaustive sources.

## Publishing online

Research code (or other code) is best stored on GitHub, this is used by millions of programmers and researchers around the world. To upload your code to GitHub, you first need to make an account here, and then a repository that hosts your code.

### Creating a repository

After logging in, click your profile picture on the top right -> "Your repositories" -> "New". Choose a name, set the visibility (do you want everyone to see your code already or wait a bit before sharing with the world?). It is also advisable to immediately add a "README" file and add a license (see next section). You can update the visibility of your repository later under the repository settings.

### GitHub Pro (free for university employees)

As a university employee, you can use your university email address to get a free "Pro" account, which gives you access to some additional features. You can find more information [here](https://education.github.com/).

### Adding code to your repository

The easiest way is add your (already) written code to the repository using the "Add file" or "+"-button.

![Adding code to your GitHub repository](/public/adding-code-github.png)

Once you uploaded your files, you need to write a "commit" message. This is basically a description of the changes that were made between the previous version of the code and the current version. Since this is the first version of the files (that is available online) you can write something like "initial".

When you plan to use GitHub more often, adding files through the web interface can become cumbersome, and it is recommended to use [GitHub Desktop](https://github.com/apps/desktop) or to use a git manager which is integrated in your IDE (integrated development environment), like [Visual Studio Code](https://code.visualstudio.com/). Git can also be used on the command line, for example, when using git from a server. A really nice tutorial on using git from the command line can be found [here](https://www.codecademy.com/learn/learn-git).

Well done! Your research code is not sitting on your own computer (or worse – deleted after publication*), but is available online for others to find. This is step one in following the [FAIR guiding principles](https://www.go-fair.org/fair-principles/). In the remainder of this guide, you will find steps that will guide you through all steps to release a "perfect" code repository. The first sections are most important, further sections provide room for further improvement, so please use this guide at your own discretion.

*NWO requires data and code to be preserved for at least 10 years, unless legal provisions or discipline-specific guidelines dictate otherwise.
...
A DOI, or Digital Object Identifier, string assigned to a document, dataset, or object to provide a permanent link. DOIs ensure that even if the URL of the content changes, the DOI link remains constant, making it a reliable way to cite and access digital materials. There are two ways to do this. Depending on how often you intend to update your code one or the other may be simpler.

1.  If you do not intend to update your code often, it is often to upload your code separately to Zenodo (or similar repository). This will then automatically generate a DOI for you, which can be included in a publication.
2.  If however, you intend to update your code frequently, it may be easier to set up an automatic link between GitHub and Zenodo, so that a new version of the code is released on Zenodo on publication of a new version on GitHub. You can find a guide on how to do so [here](https://docs.github.com/en/repositories/archiving-a-github-repository/referencing-and-citing-content).
...
The README file is the general introduction to a code repository. A good readme includes the following components (but may include other sections of course depending on the project):
...
On GitHub, the README file should ideally be named "README.md". GitHub automatically renders this file and displays it on the home page of your repository. You can find an example [here](https://github.com/numpy/numpy).
...
A huge part of commenting your code is variable naming. Good variable naming, can go a long way in making your code "self-documenting". In general, do not use abbreviations or very short variable names. This may save a couple of keystrokes but this time is easily recovered when looking at the code again months later, and will help your colleagues or other interested people even more. Modern integrated development environments also usually include autocompletion.
...
Your code is now following the FAIR principles already! However, there is always room for improvement. Futher steps are explained below.

## Documentation

Extensive documentation can help users of your code re-use your code effectively. There are some great tools out there to help you automatically build documentation from your code comments, which can be supplemented with general documentation. Sphinx is a great way to do this, and can be nicely integrated with your existing GitHub repository. Sphinx is – of course – documented using Sphinx, so an example of code documentation can be found here. This guide is not meant to be a full guide on how to use Sphinx as many useful resources can be found on the internet. However, some useful resources (here, and here) are listed, and pointers are given.
...
Documentation can be automatically updated when you push new commits to your GitHub repository. To do so, you can set up a continuous integration (CI) workflow using GitHub Actions. In a workflow file, you specify the steps to install Sphinx, build the documentation using the `sphinx-build` command, and then push the generated HTML files to the `gh-pages` branch or another branch designated for hosting your documentation. A guide on how to set this up can be found [here](https://docs.readthedocs.io/en/stable/intro/getting-started-with-sphinx.html).
...
Creating a Python package allows you to share your code with a broader audience, promoting collaboration, and ensuring reproducibility. There are two "levels" of publishing your code. First of all, you can ensure that your code is directly installable from the GitHub repository. To do so, you can follow the steps explained [here](https://packaging.python.org/en/latest/tutorials/packaging-projects/), but only up to and including the section "Configuring metadata". Here, we assume you already created a `README.md` and added a LICENSE as explained above.

In brief, you need to take the following steps, which are explained in full:
...
For a full explanation of how to do so, follow the remaining steps explained [here](https://packaging.python.org/en/latest/tutorials/packaging-projects/).

### Automatic publishing versions

If you publish new versions frequently, it may be useful to automatically release these versions to PyPi, directly from GitHub without taking manual steps. To do so, you can follow the guide [here](https://github.com/pypa/gh-action-pypi-publish).
...
For R, the most frequently used package for software testing is `testthat`, more information can be found [here](https://github.com/r-lib/testthat).
...
Increasingly, traditional scientific publications also include a section that describes where the code can be found. This can be a DOI, a link to a GitHub repository, or a link to a Zenodo repository. An example of such a section can be found here:

```markdown
Code for data cleaning and analysis is provided as part of the replication package, writting in R. The specific code that was used for this article is available at https://doi.org/10.xxxx/path/to/journal/archive, while further updates are released at https://github.com/repository/your_code.
```