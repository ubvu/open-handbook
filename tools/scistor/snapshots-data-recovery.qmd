...
If you don't have access to a Windows machine, connect to [Azure Virtual Desktop (AVD)](https://windows.cloud.microsoft/spa-signin-oidc#/devices) and follow the [instructions](./scistor-connection.qmd#windows) for mounting a project folder in Windows.
...
2. **Right-click** in the folder window and select **Properties**.
   ![Windows Step 2](/public/tools/scistor/win-2.png)
...
4. **Select the snapshot date** from which you want to retrieve the file.
   ![Windows Step 3](/public/tools/scistor/win-3.png)

5. A new File Explorer window will open, showing the files from that point in time.
   ![Windows Step 4](/public/tools/scistor/win-4.png)

6. **Copy or drag** the file back to your current folder.
   ![Windows Step 5](/public/tools/scistor/win-5.png)
...
- If you mounted `smb://scistor.vu.nl/shares`, navigate to `/Volumes/shares/<projectfolder-name>`

- If you mounted `smb://scistor.vu.nl/shares/<projectfolder-name>`, navigate to `/Volumes/<projectfolder-name>`


### Steps to Restore a File

1. **Open Terminal** and navigate to your mounted project folder:
...
```
::: {.callout-note}
Be aware that `IT-ITVO-Scistor-Demo` is a project folder used to write this documentation,
please replace this with your own project foldername.
:::
...
Linux works similarly to macOS, but the mount path depends on where you choose to mount your project folder. This example follows the default suggestion in [the handbook](./scistor-connection.qmd#other-linux-workstations).
...
::: {.callout-note}
Be aware that `IT-ITVO-Scistor-Demo` is a project folder used to write this documentation,
please replace this with your own project folder.
:::
---
...
- **Location of Snapshots**: Snapshots are always in the top level project folder, subfolders contain empty snapshots.
