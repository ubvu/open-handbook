...
If your analysis cannot be run on your PC, for example because your dataset is too large and you do not have enough storage, or your computing requirements are too heavy and the processing capacity of your machine is not big enough, you should think about using other analysis platforms: a VRE (Virtual Research Environment) such as [SciCloud](/tools/scicloud/index.qmd) or [Research Cloud](/tools/researchcloud/index.qmd); The [VU compute hub](/tools/vucomputehub/index.qmd); or a high-performance computing facility (HPC), such as [ADA](/tools/ada/index.qmd) or [Snellius](/tools/snellius/index.qmd).
...
-   On Windows using [Windows Explorer](/tools/yoda/yoda_using_windowsexplorer.qmd) or [WebDrive](/tools/yoda/yoda_using_webdrive.qmd)
-   On MacOS using [Finder](/tools/yoda/data_access_macos.qmd#mounting-the-yoda-webdav-in-finder)
-   On Linux using [Gnome Files](/tools/yoda/data_access_linux.qmd#connecting-to-the-yoda-webdav-interface-using-gnome-files) or similar.
...
| **WebDAV client**<br>[manual](/tools/yoda/introduction.qmd) | up to 100GB, up to 1000 files | PC, VRE | WebDAV can be slow when transferring a large amount of small files. It is possible to automate the transfer files using WebDAV with Python, but it would be better to use the iRODS interface, see below. |
| **iCommands or GoCommands**<br>[manual](/tools/yoda/yoda_using_icommands.qmd)  | Small to very large | PC, VRE, HPC | These command line tools can handle very large datasets and also offer many features for working with file-level metadata. It is also possible to check the integrity of uploaded and downloaded files, see the [ichksum command](https://docs.irods.org/4.3.4/icommands/user/). |
...
- [Rclone](https://rclone.org/commands/rclone_sync/), [iCommands](https://docs.irods.org/4.3.4/icommands/user/#irsync), [Gocommands](https://github.com/cyverse/gocommands/blob/main/docs/commands/sync.md) and [iBridges](https://ibridges.readthedocs.io/en/v1.0.0/sync.html) all have the option to sync a folder on your PC with Yoda. Since only changed files will be transferred this is quicker and you will be able to restart transfers when they get interrupted.
...
- If you consistently work with large datasets on campus, e.g. on your PC, SciCloud or Ada, consider storing data you are actively working with on [SciStor](/tools/scistor/index.qmd). You can store the bulk of your source data in Yoda to keep costs down and upload your results to Yoda to organize, share with external collaborators, archive and publish. 
:::
