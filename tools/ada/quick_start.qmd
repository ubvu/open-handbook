...
# Always use the job-local temporary directory:
cd $TMPDIR
...
sbatch hello.sbatch
```
where we requested a single CPU, 2 GB of memory, and the general use `defq` partition.
If you have access to department specific partitions, please use those instead as `defq` resources are pitifully limited.

`$TMPDIR` points to ADA's temporary job-local workspace. It is backed by `/tmp`, which is local disk space on the compute node, private per user, and cleaned up automatically after jobs finish. Use `$TMPDIR` for temporary data and I/O-heavy computation instead of `$HOME` on `/home`, because `/home` is NFS-mounted and slower than local disk. Copy any results you want to keep back to persistent storage before the job ends.
...
For that, you can use the same SSH proxy configuration for secure copy `scp`.
