...
# Use the job-local temporary directory (/tmp on ADA):
cd $TMPDIR
...
- `--mem` (or `--mem-per-cpu`) – memory request appropriate for your workload. On ADA, memory is scheduled explicitly, so request it deliberately.
:::
...
```sh
srun --pty --partition=<partition> --time=01:00:00 --cpus-per-task=2 --mem=4G bash
```
...
Keep `$HOME` for configuration and small files; use `$TMPDIR` for temporary working data and I/O-heavy computation on compute nodes. On ADA, `$TMPDIR` is backed by `/tmp`, which is local disk space on the node. This is usually faster than `$HOME` on `/home`, which is NFS-mounted. `$TMPDIR` is private per user and cleaned automatically after jobs finish.
...
- Interactive: `srun --pty --partition=<p> --time=01:00:00 --mem=4G bash`
