...
To open `ada.labs.vu.nl`, an interactive node such as `inter01.labs.vu.nl`, or [Open OnDemand](https://ondemand.labs.vu.nl/) directly from your computer, you must be on the VU campus network or connected to eduVPN.

You can also connect to ADA through the public stepstone `ssh.data.vu.nl`. If you configured it as a `ProxyJump` according to the [ADA login instructions](https://rdm.vu.nl/tools/ada/login.html), eduVPN is not required.
...
Open OnDemand sessions run as Slurm jobs. Request enough walltime for the transfer. The session continues until its walltime expires, even if you close the browser tab, and can be reopened from **My Interactive Sessions**. See the [Open OnDemand documentation](https://rdm.vu.nl/tools/ada/open-ondemand.html).
...
::: {.callout-note}
Keep the login node for light tasks. For a long command-line transfer, connect to `inter01`–`inter04` and use `tmux`, as described under [Large transfers](#large-transfers).
:::
...
Run a long download on `inter01`–`inter04` inside `tmux`. This lets it continue if the browser closes or the SSH connection is interrupted:
...
- [SURFfilesender](https://www.surf.nl/en/services/storage-data-management/surffilesender)
- [SURFfilesender CLI client](https://servicedesk.surf.nl/wiki/spaces/WIKI/pages/198967770/SURFfilesender+CLI+client)
- [ADA Login](https://rdm.vu.nl/tools/ada/login.html)
- [ADA Quick Start](https://rdm.vu.nl/tools/ada/quick_start.html)
- [SciStor Quick Start](https://rdm.vu.nl/tools/scistor/quick_start.html)
