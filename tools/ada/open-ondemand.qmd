...
- Portal: [https://ondemand.labs.vu.nl](https://ondemand.labs.vu.nl)
...
::: {.callout-note title="Open OnDemand limits"}
Open OnDemand jobs run with the `ood` QOS. That QOS applies specific limits to how much memory, CPU, walltime, and how many jobs OOD sessions can use.

Check the current `ood` limits on ADA with:

```sh
sacctmgr show qos ood format=Name,MaxJobsPU,MaxTRESPU%30,MaxWall
```
:::
...
## System apps (RStudio, JupyterLab, VS Code etc.)

1. Open the OOD portal and click the app icon (for example RStudio, JupyterLab, or VS Code).
2. Fill in the resource form: partition, CPUs, memory, walltime (and any optional parameters).
...
Also see the apptainer quick start: [https://apptainer.org/docs/user/main/quick_start.html](https://apptainer.org/docs/user/main/quick_start.html)
...
- For inspiration, inspect the definitions of the system apps installed on ADA. These are located on the `inter03` node under `/var/www/ood/apps/sys/` path (`jupyter` and `rstudio_server` folders).
