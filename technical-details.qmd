...
#### Custom header
The navbar in the header is completely styled by the `./custom.scss`. Note that the menu items do not have a unique id, we rely on the `nth-child(n)` selector to give them their specific color, e.g.:
...
This will of course break if a menu item is added or the order is changed.