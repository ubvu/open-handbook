...
password:
```
Password is a [Data Access Password](/manuals/yoda/data_access_password/yoda_dap.qmd) generated in Yoda.
```
Option bearer_token.
...
A Yoda [Data Access Password](/manuals/yoda/data_access_password/yoda_dap.qmd) is only valid for a set amount of time. Once it has expired you should generate a new one. Run `rclone config`, choose `e) Edit existing remote` and enter the new DAP in the password section:
```
...
