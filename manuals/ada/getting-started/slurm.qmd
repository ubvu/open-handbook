...
- Determine your input, output and temporary data. Use the fast scratch directory ($TMPDIR, also see [Basic Slurm Job](../code-examples/simpleslurm.qmd)) on the compute nodes if you handle large datasets or if you have to perform intermediate I/O operations. Be aware that the scratch directory is not persistent, data will be removed from it after the job finishes.
