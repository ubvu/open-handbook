...
### GPUs in ADA
Some workloads, such as Machine Learning, require a GPU. At the moment only some of the compute nodes in the ADA general (IT) partition do not have GPUs, the availability is not sufficient for everyone at any given time. If your workload requires more extensive use of GPUs and your department has no dedicated compute nodes with GPUs, you can apply for access to the national HPC infrastructure [Snellius](./snellius.qmd) or consider [buying ADA compute nodes](#buying-dedicated-compute-nodes) with GPUs for your department. 
