...
In principle you can run any software you want on a SciCloud server. Linux OS is preferred, but Windows server is also a possibility.

Typical use cases are:

- Running web applications and services that need to be publicly accessible 24/7, for example a website showcasing of your research, a wiki, chatbot, etc.
...
## Getting started
The IT for Research Team will setup the server(s) with the OS of your choice (Windows or Linux) and will set up a root account for you:

- For access to a Linux server you will need to provide a personal SSH key protected with a passphrase. You will be able to access SSH fromn home. 

- For access to Windows servers use Remote Desktop Protocol (RDP), only via 🔒[eduVPN institute access](https://vu.eduvpn.nl).

Root access to the server allows you to install akk the software you need. Note that IT for Research can help you get started and will automatically install OS security updates, but you are responsible for installing and maintaining software yourself.
