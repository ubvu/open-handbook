...
Selecting data means making choices about what to keep for the long term, and what data to [archive securely and what data to publish openly](../guides/publish-and-share.html#selecting-an-archive). This means that you have to decide whether your dataset contains data that need to be removed or separated. Reasons to exclude data from publishing include (but are not limited to):

* data are redundant
* data concern temporary byproducts which are irrelevant for future use
* data contain material that is sensitive, for example personal data in the sense of the [GDPR](../guides/plan-and-design.html#gdpr-privacy), like consent forms, voice recordings, DNA data; state secrets; data that are sensitive to competition in a commercial sense. These data need to be separated from other data and archived securely
...
* Raw or cleaned data (if the cleaned data has been archived, the [provenance](../guides/process-and-analyze.html#data-provenance) documentation is also required)
* [Project documentation](../guides/document-and-preserve.html#data-documentation)
* [Codebook](../guides/document-and-preserve.html#codebooks) or [protocol](../guides/collect-and-store.html#data-collection-protocols)
* [Logbook or lab journal](../guides/process-and-analyze.html#data-provenance) (when available, dependent on the discipline)
* [Software](../guides/process-and-analyze.html#data-processing) (& version) needed to open the files when no preferred formats for the data can be provided

See the section [Metadata](../guides/document-and-preserve.html#metadata) for more information about documenting your data.

Depending on the research project it may be that more than one dataset is stored in more than one repository. Make sure that each consortium partner that collects data also stores all necessary data that is required for transparency and verification. A [Consortium Agreement](../guides/plan-and-design.html#collaboration) and [Data Management Plan](../guides/plan-and-design.html#data-management-plan) will include information on who is responsible for archiving the data.