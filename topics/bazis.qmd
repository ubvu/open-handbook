...
Use your favourite SSH client to login at \<VUNETID\>@bazis.labs.vu.nl. On windows we recommend MobaXterm; Mac users can use iterm. Direct access is only possible from the Campus or from SURF. From other network locations first connect to the stepstone \<VUNETID\>@ssh.data.vu.nl, or use eduVPN Institutional Access (below). Here be provide the steps to connect to the cluster using MobaXterm.
